		...
		<Link href={`/skill/${encodeURIComponent(s.id)}`} className="gcard flex flex-col overflow-hidden border border-border bg-card">
			<div className="cover relative flex min-h-[172px] flex-col justify-between gap-3.5 overflow-hidden p-4" style={{ background: bg, color: s.hasDemo ? "#fff" : fg }}>
				...
				<div className="relative flex items-start justify-between gap-2.5">
					<div className={`pt-1 text-[11px] font-normal tracking-[.14em] uppercase ${s.hasDemo ? "opacity-90 [text-shadow:0_1px_14px_rgba(0,0,0,.55)]" : "opacity-60"}`}>Inno / Skill {s.num}</div>
					<div className={`num shrink-0 text-[42px] leading-[.85] font-normal tracking-[0] ink ${s.hasDemo ? "[text-shadow:0_1px_14px_rgba(0,0,0,.55)]" : ""}`}>{s.num}</div>
				</div>
				<div className="relative min-w-0">
					<div className={`ink text-[22px] leading-[1.1] font-normal tracking-normal [overflow-wrap:anywhere] ${s.hasDemo ? "[text-shadow:0_1px_14px_rgba(0,0,0,.55)]" : ""}`}>{s.name}</div>
					<div className={`mt-[7px] text-[11px] font-normal tracking-[.12em] uppercase ${s.hasDemo ? "opacity-90" : "opacity-70"}`}>{s.category}</div>
					{!s.hasDemo && (
				...
				{s.hasDemo && <span className="absolute right-3.5 bottom-3.5 z-[2] rounded-[3px] bg-black/70 px-[7px] py-[3px] text-[11px] font-normal tracking-[.1em] text-white">▶ DEMO</span>}
			</div>
			<div className="flex flex-1 flex-col gap-2 p-[18px]">
				<div className="flex items-center justify-between gap-2.5">
					<div className="truncate text-[11.5px] font-normal tracking-[.12em] text-muted-foreground uppercase">{s.refText || "inno-agent-hub"}</div>
					<Badges s={s} />
				</div>
				<div className="iname ink text-[21px] leading-[1.15] font-normal tracking-normal break-words">{s.name}</div>
